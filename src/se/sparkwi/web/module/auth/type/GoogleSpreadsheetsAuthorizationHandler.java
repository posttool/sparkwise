...
public class GoogleSpreadsheetsAuthorizationHandler extends OAuth1BaseHandler
{
	...
	public OBJECT get_user_profile_info(String access_token,String access_token_secret) throws WebApplicationException
	{
		...
		try{
			profile 	  = api.call("/spreadsheets/private/full");
		}catch(WebApplicationException e)
		{
			if(e.getCode() == 403)
			{
				throw new WebApplicationException("No spreadsheets account for this user. Please connect with a user that has an spreadsheets account.");
			}
			else
				throw e;
		}
		...
		if(uname.startsWith("Available Spreadsheets - "))
			uname = uname.substring(25);
