...
public class Facebook_GetComments extends WidgetDefinition
{
	...
	protected OBJECT get_data(ConnectionApi api, OBJECT selector_values, OBJECT props) throws WebApplicationException
	{
		...
		OBJECT response = api.call("/" + selector_values.get("page") + "/posts", new OBJECT("fields", "comments", "since",
				dd.format(since), "until", dd.format(until), "limit", "0"));
		int total = 0;
		...
		for (int i = 0; i < posts.size(); i++)
		{
			...
			if (p.containsKey("comments"))
			{
				total += p.O("comments").I("count");
			}
		}

		return new OBJECT("data_date", since, "value", total);
