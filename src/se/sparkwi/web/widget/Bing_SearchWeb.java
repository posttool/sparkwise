...
public class Bing_SearchWeb extends WidgetDefinition
{
	...
	protected OBJECT get_data(ConnectionApi api, OBJECT selector_values, OBJECT props) throws WebApplicationException
	{
		...
		OBJECT o = api.call("http://api.bing.net/json.aspx", new OBJECT("AppId", BingApp.ID, "Query", "\"" + query + "\"", "Market",
				market, "Sources", "web", "Version", "2.0", "Web.Offset", "0", "Web.SortBy", "Relevance"));
		OBJECT web = o.O("SearchResponse.Web");
		...
		if (count != 0)
		{
			...
			if (results != null)
				for (int i = 0; i < results.size(); i++)
				{
					...
					String url = r.S("Url");
					Date date = DateUtil.parseOrNull(r.S("DateTime"));
