...
public class Facebook_GetAlbumLikes extends WidgetDefinition
{
	...
	protected OBJECT get_selector_data(ConnectionApi api, OBJECT selector_values, String name) throws WebApplicationException
	{
		...
		if (name.equals("album") && selector_values.get("page") == null)
		{
		...
		{
			OBJECT response = api.call("/" + selector_values.get("page") + "/albums");
			ARRAY album_list = response.A("data");
			ARRAY album_names = new ARRAY();
			ARRAY album_ids = new ARRAY();

			for (int i = 0; i < album_list.size(); i++)
			{
				OBJECT p = (OBJECT) album_list.get(i);
				album_names.add(p.S("name"));
				album_ids.add(p.S("id"));

			}
			return new OBJECT("selector_name", name, "display_values", album_names, "values", album_ids);
		}
		return null;
	}
	...
	protected OBJECT get_data(ConnectionApi api, OBJECT selector_values, OBJECT props) throws WebApplicationException
	{
		...
		OBJECT likes = api.call("/" + selector_values.get("album") + "/likes", new OBJECT("limit", "0"));
