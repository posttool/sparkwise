...
public class Facebook_GetFanPosts extends WidgetDefinition
{
	...
	protected OBJECT get_data(ConnectionApi api, OBJECT selector_values, OBJECT props) throws WebApplicationException
	{
		...
		OBJECT response = api.call("/" + selector_values.get("page") + "/feed",
				new OBJECT("fields", "from,type,message", "since", dd.format(since), "until", dd.format(until), "limit", "0"));
		...
		for (int i = 0; i < posts.size(); i++)
		{
			...
			{
				String id = p.S("from.id");
				if (!id.equals(selector_values.S("page")))
					count++;
			}
		}

		return new OBJECT("data_date", since, "value", count);
	}
